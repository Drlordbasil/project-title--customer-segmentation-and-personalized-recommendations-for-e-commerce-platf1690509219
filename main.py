...
def gather_data():
    ...
    merged_data = pd.merge(data_source_1, data_source_2, on='customer_id', how='inner')
    merged_data = pd.merge(merged_data, data_source_3, on='customer_id', how='inner')
...
def recommend_products(data):
    ...
    user_item_matrix = data.pivot_table(index='customer_id', columns='product_id', values='purchase_amount', fill_value=0)
    ...
    for i, customer_id in enumerate(user_item_matrix.index):
        ...
        top_product_indices = np.argsort(predicted_purchase_amounts)[-5:]  # Select top 5 products
...
def evaluate_recommendations(actual_recommendations, predicted_recommendations):
    ...
    average_precision = average_precision_score(actual_recommendations, predicted_recommendations)
...
def create_dashboard(data, recommendations):
    ...
    sns.scatterplot(x='purchase_amount', y='time_spent_on_site', hue='segment', data=data)
    ...
    bar_colors = ['blue' if p in customer_purchases else 'grey' for p in recommended_products]
    ax.bar(recommended_products, np.ones(len(recommended_products)), color=bar_colors)
...
def main():
    ...
    # Step 4: Evaluation and Performance Metrics
    actual_recommendations = data[data['is_purchased'] == 1]['product_id']  # Actual purchases as recommendations
    predicted_recommendations = [p for recommendations in recommendations.values() for p in recommendations]
    precision, recall, f1, average_precision = evaluate_recommendations(actual_recommendations, predicted_recommendations)
